...
def print_manual_instructions():
    """Prints instructions for manually starting Biggie services."""
    print(f"\n{Colors.BLUE}{Colors.BOLD}🚀 Manual Startup Instructions{Colors.ENDC}\n")

    print("To start Biggie, you need to run these commands in separate terminals:\n")
    ...
    print("Once all services are running, access Biggie at: http://localhost:3000\n")
...
def main():
    ...
    if "--help" in sys.argv:
        ...
        print("Manage Biggie services based on your setup method")
        print("\nOptions:")
    ...
    if setup_method == "manual":
        ...
        print("Managing all Biggie services with Docker Compose...\n")
        ...
        if is_up:
            action = "stop"
            msg = "🛑 Stop all Biggie services? [y/N] "
        else:
            action = "start"
            msg = "⚡ Start all Biggie services? [Y/n] "
        ...
        if action == "stop":
            subprocess.run(["docker", "compose", "down"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Biggie services stopped.{Colors.ENDC}")
        else:
            subprocess.run(["docker", "compose", "up", "-d"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Biggie services started.{Colors.ENDC}")
            print(f"{Colors.CYAN}🌐 Access Biggie at: http://localhost:3000{Colors.ENDC}")
